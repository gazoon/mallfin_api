...
func GetSearchResultsWithDistance(shopIDs []int, location *Location, cityID *int, sortKey *string, limit, offset *uint) ([]*SearchResult, int) {
...
func GetShopDetails(shopID int, location *Location, cityID *int) *Shop {
	...
	if location == nil {
		...
		err = conn.QueryRow(`
			SELECT s.id, s.name, s.logo_small, s.logo_large, s.score, s.malls_count, s.phone, s.site, m.id, m.name, m.phone,
			  m.logo_small, m.logo_large, ST_X(m.location), ST_Y(m.location), m.shops_count
			FROM shop s
			  JOIN mall_shop ms ON s.id = ms.shop_id
			  JOIN mall m ON ms.mall_id = m.id
			WHERE s.id = $1
